...
class TestDefaultBrowser:
    ...
    async def test_start_stop(
        self, mock_async_playwright, mock_playwright, mock_context
    ):
    ...
    async def test_save_state(self, mock_async_playwright, mock_context, tmp_path):
        ...
        with patch(
            ...
            async with make_browser(
                BrowserType.DEFAULT, state_path=state_file
            ) as browser:
...
class TestStealthBrowser:
    ...
    async def test_stop_calls_stealth_exit(
...
class TestCdpBrowser:
    ...
    async def test_custom_cdp_url(self, mock_async_playwright, mock_playwright):
        ...
        with patch(
            ...
            async with make_browser(
                BrowserType.CDP, cdp_url="http://localhost:9333"
            ) as _:
                mock_playwright.chromium.connect_over_cdp.assert_called_once_with(
                    "http://localhost:9333"
                )
    ...
    async def test_context_not_closed_on_stop(
